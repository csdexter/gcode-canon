...
bool update_gcode_state(char *line) {
    ...
    /* Machine auxiliary movement comes here */
	...
	if(!currentGCodeState.axisWordsConsumed) {
		...
		if(currentGCodeState.motionMode == CYCLE) do_WCS_cycle_math(&currentGCodeState.system, get_gcode_word_real('X'), get_gcode_word_real('Y'), get_gcode_word_real('Z'), currentGCodeState.R);
		else if (!(currentGCodeState.motionMode == OFF || currentGCodeState.motionMode == STORE)) do_WCS_move_math(&currentGCodeState.system, get_gcode_word_real('X'), get_gcode_word_real('Y'), get_gcode_word_real('Z'));
	} else currentGCodeState.axisWordsConsumed = false;
    switch(currentGCodeState.motionMode) {
    case RAPID:
    	move_machine_line(currentGCodeState.system.X, currentGCodeState.system.Y, currentGCodeState.system.Z, 0, 0, 0, GCODE_FEED_PERMINUTE, GCODE_MACHINE_FEED_TRAVERSE);
    	break;
    case LINEAR:
    	move_machine_line(currentGCodeState.system.X, currentGCodeState.system.Y, currentGCodeState.system.Z, 0, 0, 0, currentGCodeState.feedMode, currentGCodeState.F);
    	break;
    case ARC:
    	...
    	break;
    case CIRCLE:
    	//move_machine_arc(currentGCodeState.system.X, currentGCodeState.system.Y, currentGCodeState.system.Z, 0.0, 360, currentGCodeState.ccw, currentGCodeState.system.plane, currentGCodeState.feedMode, currentGCodeState.F);
    	break;
    case CYCLE:
    ...
    if(have_gcode_word('M', 1, 98)) {
    	...
    	/* Seek(P) */
    	/* Push(tell) */
    	currentGCodeState.repeat = get_gcode_word_integer('L');
    }
